# Cloud Foundry on top of IBM Cloud Container Service

Cloud Foundry (BCF) is a [Cloud Foundry](https://www.cloudfoundry.org)
...
We are leveraging Fissile which has been around for a few years now and its containerization technology is fairly stable;

For development testing we've mainly been targeting [IBM Cloud Container Service](https://console.bluemix.net/containers-kubernetes/launch?env_id=ibm:yp:us-south) (using only kubernetes 1.5 features)
For more deploys on top of other kubernetes orchestration platforms, we never tried.

Table of Contents
=================

   * [Bluebosh Cloud Foundry](#bluebosh-cloud-foundry)
   * [Disclaimer](#disclaimer)
   * [Table of Contents](#table-of-contents)
   * [Deploying BCF on Vagrant](#deploying-scf-on-vagrant)
      * [Requirements](#requirements)
      * [Deploying](#deploying)
      * [Usage](#usage)
      * [Troubleshooting](#troubleshooting)
   * [Deploying BCF on Kubernetes](#deploying-bcf-on-kubernetes)
...
# Building docker images of BCF on Vagrant
...
1. Initial repo check out
    ...
    ```bash
    git clone --recurse-submodules https://github.com/bluebosh/scf
    ```
...
3. Changing the default STEMCELL
   ...
   **Example:**
   ...
   ```
   $ export FISSILE_STEMCELL_VERSION=trusty-7.ga393242-2.4
   $ export FISSILE_STEMCELL=bluebosh/fissile-stemcell-ubuntu:$FISSILE_STEMCELL_VERSION
   ```
...
* uaa does not come up correctly (constantly not ready in pod-status)
    ...
    # Delete the pv related to uaa/postgres-0
    k get pv # Find it
...
# Deploying BCF on IBM Cloud platform (kubernetes cluster)
...
## How do I publish BCF and BOSH images?
...
1. This target uses the `make` variables listed below to construct the image names and tags:
    ...
    | IMAGE_PREFIX   | bcf              | The prefix to use for image names (must not be empty) |
    | IMAGE_ORG      | bluebosh         | The organization in the image registry |
...
1. Download the [scf-cert-generator.sh](https://github.com/bluebosh/scf/blob/develop/docker-images/cert-generator/scf-cert-generator.sh) script
